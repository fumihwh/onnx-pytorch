    ...
    def gen(self, node, value_infos, initializers):
        ...
        for i in range(d):
            if i in axes:
                j = axes.index(i)
                s = ["", ""]
                if steps[j] < 0:  # need flip tensor
                    flip_dims.append(i)
                    temp_step = -steps[j]
                    # exchange the start and end points when step is negative
                    if type(starts) == str and type(ends) == str:
                        s[1] = f'{starts}[{j}] if {starts}[{j}]'
                        s[0] = f'{ends}[{j}] if {ends}[{j}]'
                    else:
                        s = [
        ...
        if len(flip_dims) > 0:
            forward_str.append(
                f"{outputs_str[0]} = {outputs_str[0]}.flip({flip_dims})")
